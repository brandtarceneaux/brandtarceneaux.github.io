...
_.filter = function(coll, test) {
    ...
    //loop using each and design a test that return booleans
    //return new array for all elements evaluated as true
    //if value passes test, push to container array
    //return new array
    var filtered = [];
...
*/
_.reject = function(coll, test) {
    var rejected = [];
    _.filter(coll, function(element, index, coll) {
...
*/
_.partition = function(coll, test) {
    var result = [[],[]];
    _.filter(coll, function(element, index, coll) {
        if(test(element, index, coll)) {
...
*/
_.unique = function(coll) {
    var uniques = [];
    for (var i = 0; i < coll.length; i++) {
        if (_.indexOf(uniques, coll[i]) === -1) {
            uniques.push(coll[i]);
        }
    }
    return uniques;
...
_.map = function(coll, test) {
    const result = [];
    _.each(coll, function(element, i, coll) {
...
_.pluck = function(arr, prop) {
    return _.map(arr, function(object) {
        return object[prop];
    });
};
...
_.contains = function(arr, val) {
    var trueFalse = false;
    _.each(arr, function(element, i, coll) {
...
*/
_.reduce = function(arr, funct, seed) {
    var prev;
      _.each(arr, function(element, i, coll) {
